# NVSentinel Phase 0 Scale Harness — Microbenchmark Results (100k nodes)

End-to-end Phase-0 scale run at **100,000 simulated nodes**. Same measurement layout as the module microbenchmarks (#1515 MongoDB, #1517 node-drainer, #1519 fault-remediation, #1525 platform-connector) so rungs diff directly.

- **Run parameters:** fatal-fraction=1.0, fatal-event=node-reboot (fleet-wide cordon convergence), 100 connectors/real-node
- **NVSentinel version:** v1.16.0
- **Generated (UTC):** 2026-08-04T18:26:00Z  ·  **Prometheus peak window:** 1h
- **Run id:** `run-1785867779-a9a24916`
- **Cordon convergence:** reached **99,999/100,000 nodes cordoned** (~10.2h to peak, held over an 11.5h monitored window)
- **Companion run:** `2026-08-06/100000` — default fatal-fraction, gpu-reset, 100 connectors/real-node, NVSentinel v1.17.0 (lighter load; ~65 cordoned, used to isolate connector-density cost)
...
| Simulated (KWOK) nodes | 100,000 (Ready 99,817) |
| Connector-pool pods | 3,200 (platform-connector density under test) |
...
| --- | --- |
| Total nodes | 100,032 |
| KWOK nodes (Ready) | 100,000 (99,817) |
| Cordoned nodes | 99,999 (peak) · 144 (report-time snapshot) |
| GPUReset CRs | 1 |
| Reset Jobs | 0 |
| Node-lock leases | 1 |
| Connector-pool pods | 3,200 |
...
| Metric | Value |
| --- | --- |
| Target nodes | 100,000 |
| Ready | 100,000 (100.0%) |
| Failed creates | 0 |
| Time-to-ready | 1778s (~29.6 min) |
| apiserver p99 during scale | 0.041s (guardrail 1s) |
| KWOK Ready at report time | 99,817 / 100,000 (99.8%) |
...
| --- | --- |
| all-verb p99 | 0.511s |
| read/LIST p99 | 0.963s |
| write p99 | 0.481s |
| LIST-nodes p99 | 9.945s |
| LIST-nodes rate | 387.3/s |
| total request rate | 1,328,549/s |
| in-flight (peak) | 204 |
| 5xx rate (peak) | 379.4/s |
...
| GPUReset latency (start->complete) | count=1 · min 44s · p50 44s · p90 44s · p99 44s · max 44s · mean 44.0s |
| Reset Jobs (succeeded/total) | 0/0 |
| janitor workqueue depth (peak) | 0 |
| janitor reconcile rate (peak) | 0.126/s |
...
| Metric | Value |
| --- | --- |
| Peak cordoned | **99,999 / 100,000** (100.0%) |
| First sample | 11,989 cordoned |
| Time to peak | ~10.2h |
| Monitored window | 11.5h (82 samples) |
| Component restarts during convergence | KOM ×100, labeler ×26, fault-remediation ×1, fault-quarantine ×0, node-drainer ×0 |
...
> Finding: KOM restarted **100×** and labeler **26×** over the 11.5h convergence — component memory/stability under a sustained full-fleet cordon is a Phase-2 follow-up.
...
| injected | 102,400 |
| acked | 102,400 |
| stored for run | 102,400 |
| accounted | 102,400 |
| missing | 0 |
| loss fraction | 0 (max 0) |
| NodeName attribution | 5,600/5,600 matched |
| verdict | ✅ PASS |
...
> REAL ceiling: apiserver LIST-nodes p99 9.945s exceeded guardrail 1.000s — large-collection LIST against apiserver/etcd is the bound for node scale (Phase 2). Overall read p99 stays low (0.963s) and in-flight peak 204, so it's LIST-fanout cost, not general control-plane saturation.

---

## 6. Pass criteria

- ✅ apiserver all-verb p99 0.511s within guardrail 1s
- ⚠️ LIST-nodes p99 9.945s (large-collection read cost at this scale)
- ✅ KWOK readiness 99.8% (kwok-controller heartbeat ceiling is a harness limit, not the SUT)
- ✅ fault-quarantine cordoned 99,999/100,000 nodes (100.0%) under a fleet-wide fatal storm — full-fleet cordon convergence achieved
- ⚠️ 5xx peaked 379.4/s — attribute vs shared-cluster background
- ✅ event delivery: loss 0, attribution 5,600/5,600
...
**Selected time range for this rung:** `2026-08-04T17:20:00Z` → `2026-08-05T07:30:00Z`

- **E2E run.log (UTC):** `2026-08-04T17:27:16Z → 18:26:38Z`
- **Cordon converge:** `2026-08-04T19:46:25Z → 2026-08-05T07:18:12Z (peak cordoned=99999)`
- **harnessctl report Prom peak window:** `1h ending 2026-08-04T18:26:00Z`
...
![cordoned](graphs/100k/cordoned.png)
...
| | |
| --- | --- |
| From | `2026-08-04T17:20:00Z` |
| To | `2026-08-05T07:30:00Z` |
...
- Raw artifacts: `results/2026-08-04/100000/report.json` + `report.md`
